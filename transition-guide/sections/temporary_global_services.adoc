...
Once a WIS centre has migrated from GTS to WIS2, it may want to stop sending its data directly on the GTS. This will require implementing the technical procedure described below, then when done, WMO Secretariat will coordinate the actions of the different parties to effectively allow the Member to stop the MSS. 

The WIS2 to GTS gateway will ensure that *only the data currently available on the GTS* and whose Member is wishing to stop its Message Switching System (MSS) will be re-published onto the GTS. 

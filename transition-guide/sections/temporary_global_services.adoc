...
To complete the migration to WIS2, a WIS centre must be able to consume data from WIS2. This requirement is described in the _Guide to WIS_, Volume II, section 1.2 _Information for the data consumer_. 
...
Data published on the GTS is re-published on WIS2 by the GTS-to-WIS2 gateways using the so-called _GTS transition topics_ (e.g., ``[origin|cache]/a/wis2/de-dwd-gts-to-wis2/data/[core|recommended]/T1/T2/A1/A2/ii/CCCC``). As a result, some data is advertised through notification messages published on both native WIS2 topics _and_ GTS transition topics. 
